...
genv.identifyexecutor=make_c_closure(function()return "Elaina","1.0.0"end)
...
genv.getsenv=make_c_closure(function(S)return{script=S}end);genv.gethui=make_c_closure(function()return CoreGui end)
...
genv.fireclickdetector=function(P)end;genv.firetouchinterest=function(t,tt,s)end
...
genv.getconnections=function(E)if type(E)~="userdata"then return{}end;local ok,C=pcall(function()return E:Connect(function()end)end);if not ok or not C then return{}end;local cs={};local c={Enabled=true,ForeignState=false,LuaConnection=true,Function=function()return C end,Thread=task.spawn(function()end),Disconnect=function()C:Disconnect()end,Fire=function(...)end,Defer=function(...)end,Disable=function(...)end,Enable=function(...)end};table.insert(cs,c);return cs end
...
genv.firesignal=function(...)end;genv.dofile=function(f)return genv.loadfile(f)()end;genv.getactors=function()return{}end;genv.run_on_actor=function(a,c)end;genv.cloneclosure=genv.clonefunction;genv.decompile=function(s)return"-- Decompilation is not available"end;genv.firetouchtransmitter=function(...)end;genv.getcallstack=function()return{}end;genv.getclipboard=function()return""end
...
genv.lz4compress=function(d)return d end;genv.lz4decompress=function(d,s)return d end
...
for k,v in pairs(genv)do if type(getgenv)=="function"then getgenv()[k]=v end end
)LUA";
